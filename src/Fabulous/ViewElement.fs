...
/// A description of a visual element
type ViewElement internal (targetType: Type, create: (IHost -> obj), update: (ViewElement voption -> ViewElement -> obj -> unit), updateAttachedProperties: (int -> ViewElement voption -> ViewElement -> obj -> unit), attribs: KeyValuePair<int,obj>[]) =
    ...
    new (targetType: Type, create: (IHost -> obj), update: (ViewElement voption -> ViewElement -> obj -> unit), updateAttachedProperties: (int -> ViewElement voption -> ViewElement -> obj -> unit), attribsBuilder: AttributesBuilder) =
    ...
    member x.Create(host) : obj =
        Debug.WriteLine (sprintf "Create %O" x.TargetType)
        let target = create host
