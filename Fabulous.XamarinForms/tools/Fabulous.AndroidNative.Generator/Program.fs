...
module Program =
    ...
    let configuration =
        { baseTypeName = "Android.Views.View"
        ...
        | Some options ->
            let result =
                ...
                |> Program.withWorkflow (fun workflow ->
                    { workflow with
                        optimize = AndroidOptimizer.optimize })
                |> Program.withReadAssembliesConfiguration (fun configuration ->
                    { configuration with
                        isTypeResolvable = AndroidConverters.isTypeResolvable
                        convertTypeName = AndroidConverters.convertTypeName
                        tryGetStringRepresentationOfDefaultValue = AndroidConverters.tryGetStringRepresentationOfDefaultValue })
